...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: HTMLImageElement | ImageBitmap, rect: { x: number; y: number; w: number; h: number }) => {
      const rx = rect.x * W;
      const ry = rect.y * H;
      const rw = rect.w * W;
      const rh = rect.h * H;
  ...
  return (
    <>
      ...
      <mesh ref={band} geometry={lineGeometry as any} material={lineMaterial as any} />
    </>
  );
}
